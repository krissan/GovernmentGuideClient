...
import { ElectionCandidateRepItem } from "../../Pages/ElectionPage";
...
import EleRidingVoteData from "./EleRidingVoteData";

interface ETCProps extends StdProps {
  eleCans:Map<number, ElectionCandidateRepItem>     ,
  setEleCans:(c: Map<number, ElectionCandidateRepItem>) => void
}
...
const EleToggleContainer:React.FC<ETCProps> = ({eleCans, setEleCans, style}) => {
    ...
    const showTab = (eleCan:ElectionCandidateRepItem) =>{
          ...
          <div style={{display:"flex", overflowX:"auto", padding:"0px 0px 30px 0px"}}>
              {
                ...
                <div hidden={!value.toggled} key={key}>
                  <div style={{display:"flex", fontWeight:"bold", height:"50px", backgroundColor: value.eleCan && gbPartyData.get(value.eleCan.partyId)?.partyColor ? '#'+gbPartyData.get(value.eleCan.partyId)?.partyColor : theme.palette.primary.dark, color:theme.palette.primary.contrastText, justifyContent:"center", alignItems:"center"}}>
                    {value.eleCan ? gbPartyData.get(value.eleCan.partyId)?.shortName+": "+value.rep.firstName+" "+value.rep.lastName : value.rep.firstName+" "+value.rep.lastName}
                  </div>
