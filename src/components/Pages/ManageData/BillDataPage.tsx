...
import { Bill, BillData, GovBody, RepBillVote, RepBiography, RepEndorsement, RepPlatform, RepReportCard } from "../../../AppContext";
...
import { getBillData, getBillVoteData, searchBill, uploadBills, uploadBillVotes } from "../../../api/bill";
...
const BillDataPage = () => {
  const [searchGBTerm, setSearchGBTerm] = useState<string>("");
  ...
  const [searchBTerm, setSearchBTerm] = useState<string>("");
  ...
  const onSearchGovBody = async () => {
    let res:Array<GovBody> = await searchGovBody(searchGBTerm); 
    setSelectedGB(null);
  ...
  const onSearchBill = async () => {
    let res:Array<Bill> = await searchBill(searchBTerm); 
    setSelectedB(null);
    if(res)
    {
      setBillList(res)
  ...
  const handleGovBodySubmit = (e:React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    onSearchGovBody();
  }
  ...
  const getData = async() => {
      ...
      if(selectedGB)
      {
        ...
        {
          message = await getBillVoteData(selectedGB, selectedB)
        }
      ...
      <div style={{flex:1}}>
          ...
          <div style={{flexDirection:"row", display:"flex"}}>
            ...
            <div style={{width:"33.3%"}}>
              ...
              {/* Search Field */}
              <form onSubmit={handleGovBodySubmit}
                style={{paddingBottom:20}}
              >
                <StandardSearchField onEnter={onSearchGovBody} onChange={(e)=>{setSearchGBTerm(e.target.value)}}/>
              </form>
              {/* Select Results */}
             ...
             <>
                ...
                <div style={{flexDirection:"column", display:"flex"}}>
                  ...
                  <form onSubmit={handleBillSubmit}
                    style={{paddingBottom:20}}
                  >
                    <StandardSearchField onEnter={onSearchBill} onChange={(e)=>{setSearchBTerm(e.target.value)}}/>
                  </form>
                  {/* Select Results */}
                  <div style={{flexDirection:"column", display:"flex"}}>
                    {
